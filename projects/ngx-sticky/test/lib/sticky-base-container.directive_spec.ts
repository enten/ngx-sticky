...
import { Subject, Subscription, asyncScheduler, timer } from 'rxjs';
...
describe('_createMonitoringObservable', () => {
  ...
  it('should returns observable which emit when config change', async () => {
    ...
    await timer(0, asyncScheduler).toPromise();
  ...
  it('should returns observable which emit when window fires events', async () => {
    ...
    expect(winAddEventListener).toHaveBeenNthCalledWith(4, 'resize', expect.any(Function), { passive: true });
    ...
    scrollHandler();
    await timer(100, asyncScheduler).toPromise();
  ...
  it('should returns observable which emit when container element scroll', async () => {
    ...
    scrollHandler();
    await timer(10, asyncScheduler).toPromise();
    scrollHandler();
    await timer(10, asyncScheduler).toPromise();
    scrollHandler();
    await timer(10, asyncScheduler).toPromise();
